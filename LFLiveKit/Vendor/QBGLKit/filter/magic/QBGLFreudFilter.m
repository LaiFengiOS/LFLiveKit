...
char * const kQBFreudFilterVertex;
char * const kQBFreudFilterFragment;

@interface QBGLFreudFilter ()

@property (strong, nonatomic) QBGLDrawable *randDrawable;

@end

@implementation QBGLFreudFilter

- (instancetype) init {
    self = [self initWithVertexShader:kQBFreudFilterVertex fragmentShader:kQBFreudFilterFragment];
    if (self) {
        [self loadTextures];
    }
    return self;
...
- (void)loadTextures {
    [super loadTextures];
    _randDrawable = [[QBGLDrawable alloc] initWithImage:[UIImage imageNamed:@"freud_rand"] identifier:@"inputImageTexture2"];
...
char * const kQBFreudFilterVertex = STRING
(
...
char * const kQBFreudFilterFragment = STRING
(
